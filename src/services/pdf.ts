...
export function generateInvoicePdf(
  ...
  for (const item of invoice.items) {
    ...
    doc.text(formatCents(item.unit_price_cents, pdfCurrency), 154, textY, {
      align: "right",
    });
    doc.text(formatCents(item.total_price_cents, pdfCurrency), 186, textY, {
      align: "right",
    });
  ...
  doc.text(
    formatCents(invoice.subtotal_cents, pdfCurrency),
    amountRight,
    y + 4,
    { align: "right" },
  );
  ...
  if (invoice.discount_cents > 0) {
    ...
    doc.text(
      `-${formatCents(invoice.discount_cents, pdfCurrency)}`,
      amountRight,
      y + 4,
      { align: "right" },
    );
  ...
  if (invoice.tax_amount_cents > 0) {
    ...
    doc.text(
      formatCents(invoice.tax_amount_cents, pdfCurrency),
      amountRight,
      y + 4,
      { align: "right" },
    );
  ...
  doc.text(
    formatCents(invoice.total_cents, pdfCurrency),
    amountRight - 1,
    y + 7.5,
    { align: "right" },
  );
  ...
  doc.text(
    "Thank you for your business. Generated by FreelanceDesk.",
    105,
    280,
    { align: "center" },
  );
...
export function generateReceiptPdf(
  ...
  doc.setTextColor(28, 25, 23);
  const title =
    payment.commission_title || commissionTitle || "Freelance Services";
  ...
  doc.setTextColor(22, 101, 52);
  doc.text(
    formatCents(payment.amount_cents, pdfCurrency),
    right - 8,
    y + 15.5,
    { align: "right" },
  );
  ...
  if (remainingBalanceCents !== undefined) {
    ...
    doc.setFont("helvetica", "bold");
    doc.text(formatCents(remainingBalanceCents, pdfCurrency), right - 8, y, {
      align: "right",
    });
    y += 10;
  } else {
  ...
  doc.setTextColor(87, 83, 78);
  doc.text(
    "This confirms that the above payment has been processed and recorded.",
    left,
    y,
  );
  ...
  doc.text("FreelanceDesk Verified Offline Record", 105, 275, {
    align: "center",
  });
